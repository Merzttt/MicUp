print("Emotes made by Gi#7731, Keybind: Comma")
...
local function CharacterAdded(Character)
	...
	for i,Emote in pairs(Emotes) do
        ...
        EmoteButton.MouseButton2Click:Connect(function()
        -- Copy emote ID to clipboard
        if setclipboard then
            setclipboard(tostring(Emote.id))
            SendNotification("Emote ID Copied", "ID: " .. tostring(Emote.id))
        else
            SendNotification("Cannot Copy", "Clipboard function not available")
        end
    end)
